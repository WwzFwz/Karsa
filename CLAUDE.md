...
4. **Pemasangan** - Dockerfile, Compose, dan CI (selesai, D88); naskahnya di
   `docs/pemasangan.md`. Belum: image-nya belum pernah dibangun di mesin selain
   ini, dan belum ada yang menerbitkannya.
...
- `docs/pemasangan.md` - memasangnya: satu image, mode kelas tanpa internet,
  dan yang perlu diperhatikan saat ter-deploy. Termasuk daftar yang belum.
...
| D87 | **Antarmukanya benar-benar dua bahasa sekarang, dan yang berlangganan pergantiannya cuma satu tempat.** Sekitar 300 kalimat di 40 berkas dibungkus `tr`; tabel yang dibangun sekali saat modul dimuat memakai `bilingual` atau `localise` supaya tidak membeku pada bahasa saat impor. `App` memanggil `useLang()` satu kali dan seluruh pohon ikut menggambar ulang. Sakelar ID/EN ditambahkan ke halaman masuk, di samping yang sudah ada di navigasi kiri dan dasbor. Nama tuts ikut diterjemahkan (`keyLabel`), dan jam di jejak perubahan mengikuti bahasa keluaran. | Diminta pemilik proyek 19 September 2026. D69 menjanjikan antarmuka dua bahasa dan yang benar-benar dua bahasa cuma 4 dari 51 berkas; sisanya tetap Indonesia sesudah orang menekan EN, yang lebih buruk daripada satu bahasa saja karena setengah layar berubah dan setengahnya tidak. Satu langganan di `App`, bukan `useLang()` di tiap komponen: dengan `tr` di 40 berkas, hook per komponen berarti 40 kesempatan lupa satu, dan yang terlupa tidak kelihatan sampai ada yang menukar bahasa. Ongkosnya satu syarat yang dicatat di situ — tidak boleh ada `memo` di antaranya, dan hari ini memang tidak ada. Tiga kelas jebakan tertangkap saat mengerjakannya, semuanya bentuk yang sama: teks yang dipakai sebagai identitas. `badgeFor(label)` di navigasi kiri membandingkan label dengan `'Ruang'`; heading palet mengelompokkan menurut kalimatnya; daftar penyedia memutuskan warna baris dengan `dataPath.startsWith('Tidak')`. Ketiganya benar sampai bahasanya berganti, lalu diam-diam salah — jadi ketiganya sekarang memakai id, rute, atau flag. Ditemukan sekalian karena verifikasi di peramban, bukan oleh `tsc`: satu kunci React yang memakai nama peserta, yang menghilangkan peserta kedua bernama sama. |
| D88 | **Tiga lubang di fondasi, ditutup bersamaan: tidak ada penangkap galat, tidak ada uji tata letak, tidak ada jalur pemasangan.** `ErrorBoundary` di dua tingkat — satu di sekeliling seluruh aplikasi, satu di sekeliling area kerja saja, dengan kunci rute supaya pindah halaman memulihkannya. 28 uji untuk `core/shape/layout.ts`, kelima bentuk, termasuk tumpang tindih, tinggi terukur, kestabilan, dan dokumen yang sempat bersiklus. Dockerfile satu kontainer, Compose untuk mode kelas, dan CI yang menjalankan perintah yang sama dengan yang dijalankan orang di laptopnya. | Diminta pemilik proyek 19 September 2026, ketiganya sekaligus. **Penangkap galat:** layar putih adalah satu-satunya kegagalan yang tidak bisa dijelaskan produk ini, karena tidak ada yang tersisa untuk dibaca, difokus, atau diumumkan — dan aturan 7 bilang aksesibilitas bukan mode yang dinyalakan, jadi jalur gagalnya juga tidak boleh. Dua tingkat karena kanvas adalah tempat paling mungkin rusak, dan rusaknya kanvas tidak boleh ikut membawa navigasi, dock, dan kode ruang. Kunci rute karena tanpa itu satu render buruk menahan pesan galat sampai akhir sesi: React tidak pernah memasang ulang boundary dengan sendirinya. Diverifikasi di peramban dengan melempar galat sungguhan, bukan dengan membacanya. **Uji tata letak:** aritmetika yang memutuskan dua kartu bertumpuk atau tidak sama sekali tidak punya uji, padahal bug yang sama sudah dua kali lolos (D54, D55). Pemeriksa tumpang tindihnya mengambil tinggi dari peta pengukuran, bukan dari `sizeOf` — diuji: memakai `sizeOf` membuat kedua sisi mengecil bersamaan dan ujinya tetap hijau. Kasus kartu lebar di dalam kolom ditambahkan sesudah memasang kembali bug D55 dan melihat semua uji lain diam. **Pemasangan:** bagian 7 menjanjikan dua pemasangan yang cuma beda konfigurasi, dan janji itu tidak bisa diperiksa selama tidak ada satu pun berkas yang membangunnya. CI menjalankan `npm run check` yang sama, bukan daftar perintahnya sendiri, karena pipeline yang memeriksa hal berbeda dari yang dijalankan orang adalah pipeline yang lama-lama diabaikan. **Batas yang diterima dan ditulis di `docs/pemasangan.md`:** `docker build` belum pernah selesai sekali pun di sini — daemon-nya mati — jadi yang diperiksa cuma asumsinya satu per satu (jalur `../dist`, mode produksi menolak jalan tanpa `KARSA_SECRET`, perintah healthcheck yang sama persis). Pembuktian pertama yang sungguhan adalah CI. |
