...
1. **Fondasi kode** - batas impor dijaga skrip, uji `core/`, `RoomContext`
   dipecah jadi alur suara per perangkat (selesai, D74) dan beberapa provider, `ollama.ts` dan
   `CanvasView.tsx` dipecah.
2. **Data sungguhan** - IndexedDB dan `YjsDocStore` di balik `DocStore` (selesai, D75), server
   satu kontainer (Hocuspocus + SQLite + room service), kehadiran lewat
   Awareness.
3. **Pengalaman** - latensi model, mode Menyimak, VAD, uji NVDA, uji pengguna.
